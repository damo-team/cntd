...
export default class TagSearcher extends Component {
    ...
    if (object) {
      ...
      if (object.tagList) {
        ...
        if (tagCode) {
          ...
          tagsMap[entityCode].tagCode = focusItem
            ? focusItem.tagCode
            : '';
        }
      } else if (entityCode) {
  ...
  render() {
    ...
    return (
      <div className="j-kit-tagSearcher">
        <Input.Search
          placeholder={this.props.searchPlaceholder || '使用 实体.标签 进行模糊搜索'}
          value={keyword}
          suffix={keyword ? ( <Icon type="close-circle" onClick={e => this.clearSearch()} /> ) : null}
          onChange={(e) => {
          this.setState({keyword: e.target.value})
        }}
          onSearch={(e) => this.handleSearch(this.state.keyword)}/>
        <div className="j-kit-elem-oltList">
          {this.props.keyword ? (<Alert message="通过判定式定位" type="info" closable onClose={e => this.clearSearch()}/>) : null}
          <Collapse
            accordion
              ...
              return (
                <Collapse.Panel
                  ...
                  <ItemDom key={entity.entityCode} doAction={doDrag} type="entity" data={entity}>
                    <div className="j-kit-elem-icon-entity"></div>
                  ...
                  <ul>
                    {dimsMap[entity.entityCode] && dimsMap[entity.entityCode].map(dim => {
                      return (
                        <li>
                          <ItemDom key={dim.code} doAction={doDrag} type="dim" data={dim}>
                            <div className="j-kit-elem-icon-dim"></div>
                            <Tooltip
                              placement="right"
                              title={`${entity.entityType === 'object' && '实体' || '关系'}(${entity.entityName})下可用的标签`}>
                              <Icon type="pushpin-o" className="j-kit-elem-icon-pushpin-o"/>
                            </Tooltip>
                            <span title={dim.name}>
                              {dim.name}
                            </span>
                  ...
                  <ul>
                    {tagsMap[entity.entityCode] && (tagsMap[entity.entityCode].loading
                      ...
                      : tagsMap[entity.entityCode].list.length
                        ? tagsMap[entity.entityCode].list.map(tag => {
                          return (
                            <li>
                              <ItemDom key={tag.tagCode} doAction={doDrag} type="tag" data={tag}>
                                <div className="j-kit-elem-icon-tag"></div>
                                <Tooltip
                                  placement="right"
                                  title={`${entity.entityType === 'object' && '实体' || '关系'}(${entity.entityName})下可用的标签`}>
                                  <Icon type="tag-o" className="j-kit-elem-icon-tag-o"/>
                                </Tooltip>
